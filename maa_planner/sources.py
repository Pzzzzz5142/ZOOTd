...
from .models import Activity, ActivityStage, OfficialWindow, StageEfficiency
from .util import (
    atomic_write_bytes,
    atomic_write_json,
    fixed_timezone,
...
class HttpCache:
    ...
    def fetch_text(self, key: str, url: str, *, max_stale: timedelta) -> tuple[str, FetchResult]:
        result = self.fetch(key, url, accept="text/html, text/plain", max_stale=max_stale)
        try:
            return result.body.decode("utf-8"), result
        except UnicodeDecodeError as exc:
            raise SourceError(f"response is not UTF-8: {url}") from exc
...
class _VisibleTextParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        if tag in self._SKIP:
            self.skip_depth += 1
        if not self.skip_depth and tag in self._BLOCKS:
            self.parts.append("\n")
    ...
    def text(self) -> str:
        ...
        for line in "".join(self.parts).splitlines():
            compact = re.sub(r"\s+", " ", html.unescape(line)).strip()
            if compact:
...
_TIME_RANGE = re.compile(
    r"(?P<sm>\d{1,2})\s*月\s*(?P<sd>\d{1,2})\s*日\s*"
    r"(?P<sh>\d{1,2})\s*[:：]\s*(?P<smin>\d{2})"
    r"\s*(?:-|－|—|–|~|～|至)\s*"
    r"(?P<em>\d{1,2})\s*月\s*(?P<ed>\d{1,2})\s*日\s*"
    r"(?P<eh>\d{1,2})\s*[:：]\s*(?P<emin>\d{2})"
)


def _activity_name_from_title(title: str) -> str:
    match = re.search(r"(?:SideStory|故事集|主题曲)?[「『](.+?)[」』]", title, re.IGNORECASE)
    if match:
        return match.group(1).strip()
    return title.strip()


def _local_datetime_near_article(
    reference: datetime,
    month: int,
    day: int,
    hour: int,
    ...
    for year in (reference.year - 1, reference.year, reference.year + 1):
        try:
            candidates.append(datetime(year, month, day, hour, minute, tzinfo=_CHINA_TZ))
        except ValueError:
            continue
    if not candidates:
        raise SourceError("official bulletin contains an invalid stage date")
    return min(candidates, key=lambda value: abs((value - reference).total_seconds()))


def extract_official_windows(
    page_html: str,
    *,
    article_id: str,
    article_title: str,
    ...
    activity_name = _activity_name_from_title(article_title)
    published_local = published_at.astimezone(_CHINA_TZ)
    stage_context = False
    stage_window_seen = False
    windows: list[OfficialWindow] = []
    seen: set[tuple[str, str]] = set()

    for line in lines:
        ranges = list(_TIME_RANGE.finditer(line))
        if "活动关卡开启时间" in line or "活动关卡开放时间" in line:
            stage_context = True
            stage_window_seen = False
        direct_stage_label = "关卡开放时间" in line or "关卡开启时间" in line
        # A stage heading is followed by one or more consecutive bullet lines.
        # Once a stage range was seen, any intervening non-range line starts a
        # new section.  Explicit non-stage time labels also terminate the
        # context, even when they immediately contain their own range.
        non_stage_time_label = bool(
            re.search(r"(?:商店|兑换|售卖|时装|家具|寻访|相谈室).*时间", line)
            or re.search(r"(?:商店|兑换|售卖|时装|家具|寻访|相谈室).*开放", line)
        )
        if stage_context and not direct_stage_label:
            if non_stage_time_label or (stage_window_seen and not ranges):
                stage_context = False
        for match in ranges:
            start = _local_datetime_near_article(
                published_local,
                int(match["sm"]),
                int(match["sd"]),
                int(match["sh"]),
            ...
            end_year = start.year + (1 if int(match["em"]) < int(match["sm"]) else 0)
            end = datetime(
                end_year,
                int(match["em"]),
                int(match["ed"]),
            ...
            # Bulletins conventionally print the final valid minute as :59;
            # normalize that to the next exact, exclusive server boundary.
            if int(match["emin"]) == 59:
                end += timedelta(minutes=1)
            if end <= start:
                raise SourceError("official bulletin contains a non-positive stage window")
            is_stage = direct_stage_label or stage_context > 0
            kind = "stage" if is_stage else "event"
            label_text = line[: match.start()].strip(" ：:◆") or kind
            label = f"{kind}:{label_text}"
            identity = (isoformat(start), isoformat(end))
            ...
            if is_stage:
                stage_window_seen = True
    return windows


def fetch_official_windows(
    cache: HttpCache,
    *,
    ...
    max_pages: int,
    max_articles: int,
    cache_max_stale: timedelta,
    article_lookback: timedelta,
) -> tuple[list[OfficialWindow], dict[str, Any]]:
    articles: list[dict[str, Any]] = []
    source_hashes: list[str] = []
    all_network_validated = True
    for page in range(1, max_pages + 1):
        url = list_url_template.format(page=page)
        payload, fetched = cache.fetch_json(f"official-news-{page}", url, max_stale=cache_max_stale)
        source_hashes.append(fetched.sha256)
        all_network_validated = all_network_validated and fetched.network_validated
        if not isinstance(payload, dict) or payload.get("code") != 0:
            raise SourceError("official activity list returned an invalid envelope")
        data = payload.get("data")
        page_articles = data.get("list") if isinstance(data, dict) else None
        if not isinstance(page_articles, list):
            raise SourceError("official activity list has no data.list")
        articles.extend(item for item in page_articles if isinstance(item, dict))
        if isinstance(data, dict) and data.get("end") is True:
            break

    windows: list[OfficialWindow] = []
    inspected = 0
    for article in articles:
        if inspected >= max_articles:
            break
        title = article.get("title")
        article_id = article.get("cid")
        published = article.get("displayTime")
        if not isinstance(title, str) or not isinstance(article_id, str) or not isinstance(published, int):
            continue
        if not re.search(r"SideStory|故事集|活动预告", title, re.IGNORECASE):
            continue
        published_at = datetime.fromtimestamp(published, UTC)
        if utc_now() - published_at > article_lookback:
            continue
        inspected += 1
        article_url = article_url_template.format(cid=urllib.parse.quote(article_id, safe=""))
        text, fetched = cache.fetch_text(
            f"official-article-{article_id}", article_url, max_stale=cache_max_stale
        )
        source_hashes.append(fetched.sha256)
        all_network_validated = all_network_validated and fetched.network_validated
        windows.extend(
            extract_official_windows(
                text,
                article_id=article_id,
                article_title=title,
                article_url=article_url,
    ...
    evidence = {
        "network_validated": all_network_validated,
        "articles_inspected": inspected,
        "sha256": sha256_bytes("".join(source_hashes).encode()),
    }
    return windows, evidence


def fetch_official_bulletin_windows(
    cache: HttpCache,
    *,
    list_url: str,
    ...
    def validate_list(value: Any) -> None:
        if (
            ...
            or not isinstance(value["data"].get("list"), list)
        ):
            raise SourceError("official bulletin list returned an invalid envelope")

    def validate_detail(value: Any) -> None:
        if (
            not isinstance(value, dict)
            or value.get("status") != 0
            or value.get("code") != 0
            ...
            raise SourceError("official bulletin detail returned an invalid envelope")

    payload, list_fetch = cache.fetch_json(
        "official-bulletin-list",
        list_url,
        max_stale=cache_max_stale,
    ...
    if (
        ...
        or not isinstance(payload["data"].get("list"), list)
    ):
        raise SourceError("official bulletin list returned an invalid envelope")

    windows: list[OfficialWindow] = []
    source_hashes = [list_fetch.sha256]
    article_evidence: list[dict[str, Any]] = []
    all_network_validated = list_fetch.network_validated
    inspected = 0
    matching_articles = 0
    now = utc_now()
    for item in payload["data"]["list"]:
        if inspected >= max_articles:
            break
        if not isinstance(item, dict) or item.get("category") != 1:
            continue
        cid = item.get("cid")
        published_raw = item.get("displayTime")
        if not isinstance(cid, str) or not isinstance(published_raw, str):
            continue
        try:
            published_local = datetime.strptime(published_raw, "%Y-%m-%d").replace(tzinfo=_CHINA_TZ)
        except ValueError:
            continue
        published_at = published_local.astimezone(UTC)
        if now - published_at > article_lookback:
            continue
        inspected += 1
        detail_url = detail_url_template.format(cid=urllib.parse.quote(cid, safe=""))
        detail, fetched = cache.fetch_json(
            f"official-bulletin-{cid}",
            detail_url,
            max_stale=cache_max_stale,
        ...
        article_evidence.append(
            {
                ...
                "fetched_at": getattr(fetched, "metadata", {}).get("fetched_at"),
                "network_validated": fetched.network_validated,
            }
        )
        if (
            not isinstance(detail, dict)
            or detail.get("status") != 0
            or detail.get("code") != 0
            or not isinstance(detail.get("data"), dict)
        ):
            raise SourceError(f"official bulletin {cid} returned an invalid envelope")
        data = detail["data"]
        header = data.get("header")
        content = data.get("content")
        if not isinstance(header, str) or not isinstance(content, str):
            continue
        if "SideStory" not in header and "故事集" not in header:
            continue
        matching_articles += 1
        article_windows = extract_official_windows(
            content,
            article_id=cid,
        ...
        # A store/costume window is not evidence that battle stages are open.
        windows.extend(window for window in article_windows if window.label.startswith("stage:"))
    ...
    return windows, {
        ...
        "sha256": sha256_bytes("".join(source_hashes).encode()),
        "source": "hypergryph-game-bulletin",
        "list": {
            "url": list_url,
            "sha256": list_fetch.sha256,
            "fetched_at": getattr(list_fetch, "metadata", {}).get("fetched_at"),
            "network_validated": list_fetch.network_validated,
        },
        "articles": article_evidence,
    }
