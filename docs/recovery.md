...
自动维修的操作细则统一维护在 [恢复 scope 与 FAQ](llm-recovery-scope.md)。启动 prompt 只保留高层任务、契约路径与 SHA-256、证据信任边界和结构化输出要求；不内嵌 Markdown 全文，也不另写一份细则。代理在行动前（含续跑）从项目根目录读取完整契约并核对摘要，adapter 仍在调用 SDK 前校验契约与事故身份。

以后新增或修改恢复约束，优先修改该契约并提升版本，同步 controller 的契约版本和相关测试；需要程序强制执行的边界同时更新确定性校验。不要再把约束复制进 Python prompt。测试应验证契约引用、事故证据与身份校验，而不是要求 prompt 包含操作细则。
